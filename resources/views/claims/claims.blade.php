        ...
        <form method="GET" action="{{ route('claims.my-claims') }}">
            <div class="row g-3 align-items-end">
                <div class="col-md-5">
                    ...
                    <input type="text"
                           name="q"
                           class="form-control form-glass rounded-3"
                           value="{{ request('q') }}"
                ...
                <div class="col-md-2">
                    <button type="submit" class="btn btn-light w-100 fw-bold" style="border-radius: 8px;">
                        Filter
                    </button>
                </div>
            </div>
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="claim-photo-box me-2 d-flex align-items-center justify-content-center">
                                        @if(!empty($claim->originalItem) && !empty($claim->originalItem->foto_barang))
                                            <img src="{{ asset('storage/' . $claim->originalItem->foto_barang) }}" alt="Foto Barang">
                                        @else
                            ...
                            <td>
                                <span class="badge bg-white bg-opacity-10 fw-normal" style="font-size: 0.7rem;">
                                    {{ $claim->category ?? '-' }}
                                </span>
                            </td>
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-1">
                                    ...
                                    <a href="{{ route('claims.print', $claim->id) }}"
                                       ...
                                       title="Cetak Result PDF">
                                        <i class="fas fa-print fa-xs"></i>
                                    </a>

                                    @if($claim->status !== 'taken')
                                        <form action="{{ route('claims.mark-taken', $claim->id) }}"
                                              method="POST"
                                              class="d-inline"
                                              onsubmit="return confirm('Barang sudah Anda terima? Barang akan dihapus dari daftar temuan publik.');">
                                            @csrf
                                            @method('PATCH')
