...
<head>
    ...
    <style>
        ...
        .header h2 { margin: 0; text-transform: uppercase; font-size: 18px; letter-spacing: 1px; color: #1a202c; }
        .header p { margin: 5px 0 0; font-size: 12px; color: #718096; }
        
        /* Garis Hijau/Teal Pemisah Khas Dashboard */
        .divider { height: 3px; background-color: #4fd1c5; margin: 20px 0 30px 0; border-radius: 2px; }

        /* Layout Data */
        .content { margin: 0 10px; }
        .data-row { margin-bottom: 20px; border-bottom: 1px solid #edf2f7; padding-bottom: 8px; }
        ...
        /* Foto */
        .photo-section { margin-top: 40px; text-align: center; page-break-inside: avoid; }
        .photo-box { 
...
<body>
    ...
    <div class="content">
        ...
        <div class="data-row">
            ...
            <div class="value">{{ \Carbon\Carbon::parse($claim->date_found)->format('d F Y') }}</div>
        </div>

        <div class="data-row">
            <span class="label">Status Pengajuan</span>
            <div class="value status-{{ $claim->status }}">
                <span style="font-weight: bold;">{{ ucfirst($claim->status) }}</span>
            </div>
        </div>
        ...
        <div class="data-row">
            ...
            <div class="value" style="background: #f7fafc; padding: 10px; border-radius: 5px;">{{ $claim->claim_reason }}</div>
        </div>
    </div>
    ...
    <div class="footer">
        ...
        <br>Harap bawa bukti fisik jika status disetujui.
    </div>
